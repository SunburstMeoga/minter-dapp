<template>
    <div class="flex flex-col justify-start items-center bg-black pb-20">
        ...
        <!-- <van-popup v-model:show="showBottomPopup" class="bg-black" round position="bottom" :style="{ 'max-height': '78%' }">
            <div class="bg-black flex flex-col items-center w-full pb-4 text-white">
                ...
                <div class="w-full">
                    <div class="w-full flex flex-col items-center mb-2" v-for="(  item, index  ) in   addressList  "
                        :key="index">
                ...
                <div class="w-11/12 flex justify-between items-center mb-6">
                    <div @click="currentPoint = index"
                        class="w-5/12 rounded-lg py-1.5 text-center border border-primary-color"
                        :class="currentPoint == index ? 'operating-button text-white' : 'text-primary-color '"
                        v-for="(  item, index  ) in   pointLsit  " :key="index">
                        {{ item.title }}
                    </div>
                </div>
                <div class="w-11/12 operating-button text-center py-2.5 rounded-lg" @click="handleConfirmBuy">确认购买</div>
